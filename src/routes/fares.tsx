...
// Fares are stored in cents — format for display
function formatRand(cents: number) {
  return `R ${(cents / 100).toFixed(2)}`
}

function FaresPage() {
  const fareZones = useQuery(api.fares.getAll) ?? []
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        {/* Metroplus — featured */}
        <div className="rounded-xl border border-blue-400/25 bg-blue-500/3 p-6 flex flex-col items-center text-center">
          ...
          <p className="text-sm text-slate-400 mb-4 flex-1">
            The easiest way to travel. Tap in and out at gates. Get 15% off all
            single fares.
          </p>
        ...
        {/* Paper Tickets */}
        <div className="rounded-xl border border-white/10 bg-white/3 p-6 flex flex-col items-center text-center">
          ...
          <table className="w-full text-sm text-left">
            <thead>
              <tr className="border-b border-white/10 bg-white/3">
                {['Zone', 'Distance', 'Examples', 'Single', 'Return', 'Monthly'].map(
                  (h, i) => (
                    <th
                      key={h}
                      className={`px-6 py-4 text-xs font-semibold uppercase tracking-wider ${
                        i >= 3 ? 'text-right' : ''
                      } ${i === 5 ? 'text-blue-400' : 'text-slate-500'}`}
                    >
                      {h}
                    </th>
                  ),
            ...
            <tbody>
              {fareZones === undefined ? (
                ...
                fareZones.map((zone, _) => (
                  <tr
                    key={zone._id}
                    className="border-b border-white/6 hover:bg-white/6 transition-colors"
                  >
                    ...
                    <td className="px-6 py-4 text-right font-medium text-slate-300">
                      {formatRand(zone.singleFare)}
                    </td>
                    <td className="px-6 py-4 text-right font-medium text-slate-300">
                      {formatRand(zone.returnFare)}
                    </td>
                    <td className="px-6 py-4 text-right font-bold text-blue-400">
                      {formatRand(zone.monthlyFare)}
                    </td>
                  </tr>
                ))
              )}
      ...
      {/* Info notice */}
      <div className="flex gap-3 p-4 rounded-xl bg-white/4 border border-white/10 text-sm text-slate-300">
